...
- With the Internet, we realized that letting apps and machines handle authentication on their own was a bad idea.
  - lots of duplicated passwords and lots of logins
...
- Big IT players assembled a committee to solve the problem. The solution was SAML
...
- The OAuth protocol was created specifically to handle delegated authorization scenarios, like when you let a random application post something on Facebook as if it was you.
- It is a standard that provides authorization in the cloud with consent.
...
<Notes>
 - OpenID Connect is a concrete standard protocol for authenticating end users derived on top of the oAuth 2.0 framework/protocol. As such is also called a profile of OAuth 2.0. There are others.
...
- An authentication process usually relies on some form of proof. For example, if you go to the bank and try to withdraw money from your account, the clerk might ask you for an ID (an official document) to check who you are.
...
For a concrete example, imagine a situation where some people get tickets for a concert. In this case,
more often than not, the establishment will not be interested in their identity (i.e., on who they are). What
they care about is whether they are authorized or not to attend the show. To prove that they have the right
to be there, instead of using an ID or a passport, they would use a ticket that contains no information
about you.
...
import oauth2slide from "./assets/oauth2_slide.png";

<img src={oauth2slide} width="100%" />

<Notes>

- It uses REST architectural style and JSON to request and retrieve tokens to grant access to cloud resources.
- It can securely issue and validate tokens for services on the Internet.
...
- This is what should happen for the Client App to obtain an access token and to be able to access a protected resource, such an API endpoint, a particular address etc.
- In order to obtain this access token, the Client needs to present a valid grant type to the authorization server.
- Also reffered to as flows, the participants interact through them.
...
They are both called redirect flows, because they operate through Redirect URLs.
...
<Highlighter language="http">{`HTTP/1.1 302 Found
...
<img src={login_consent} width="100%" />

</ul>

<Notes>
  At the authorisation server, the user will typically be authenticated by
  checking if they have a valid session (established by a browser cookie), and
  in the absence of that, by prompting the user to login. After that the server
  will determine if the client is to be authorised or not, by asking the user
  for their consent, by applying a rule or policy, or by some other mean.
</Notes>
...
<Notes>
The authorisation server will then call the client redirect_uri with an authorisation code (on success) or an error code (if access was denied, or some other error occurred, such a malformed request was detected).
...
<ul>
  <li>Client requests a response using the Authorization Code at the Tokenn Endpoint</li>

<Highlighter language="http">{`POST /token HTTP/1.1
...
<Highlighter language="http">{`HTTP/1.1 200 OK
...
<Highlighter language="http">{`GET /resource/v1 HTTP/1.1
...
<Highlighter language="http">{`eyJhbGciOiJSUzI1NiIsImtpZCI6IjFlOWdkazcifQ.ewogImlzc
